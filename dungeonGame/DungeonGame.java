...
/**
 * Question: The demons had captured the princess (P) and imprisoned her in the bottom-right corner of a dungeon.
...
public class DungeonGame {
    ...
    private int maxMinHp = Integer.MIN_VALUE;
    ...
    public int calculateMinimumHP(int[][] dungeon) {
        ...
        exploreDungeon(dungeon, new Coord(0, 0), 0, 0);
        if (maxMinHp < 0) {
            return 1 - maxMinHp;
        }
        return 1;
    ...
    private void exploreDungeon(int[][] dungeon, Coord loc, int hp, int minHp) {
        hp += dungeon[loc.row][loc.offset];
        minHp = Math.min(hp, minHp);
        if (minHp < maxMinHp) {
            return;
        }
        if (loc.row == dungeon.length - 1 && loc.offset == dungeon[dungeon.length - 1].length - 1) {
            maxMinHp = Math.max(minHp, maxMinHp);
            return;
        }
        for (Coord c : adj(dungeon, loc)) {
            exploreDungeon(dungeon, c, hp, minHp);
        }
    }

    private List<Coord> adj(int[][] dungeon, Coord loc) {
        if (loc.row == dungeon.length - 1) {
            return Arrays.asList(new Coord(loc.row, loc.offset + 1));
        } else if (loc.offset == dungeon[loc.row].length - 1) {
            return Arrays.asList(new Coord(loc.row + 1, loc.offset));
        }
        return Arrays.asList(new Coord(loc.row, loc.offset + 1), new Coord(loc.row + 1, loc.offset));
    }
    ...
    public void testCase2() {
        ...
        System.out.println(String.format("testCase2 took % ms", System.currentTimeMillis() - start));
    ...
    public static void main(String[] args) {
        Result result = JUnitCore.runClasses(DungeonGame.class);
